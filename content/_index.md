...
*   Nov 6th, 2013: Release of variant tools 2.1.0, which adds a few useful features such as functions [`genotype()`][1] and [`sample()`][2] SQL function, and the `--as` option to command `vtools use`. 
*   Oct 9, 2013: Release of variant tools 2.0.1, which is a maintenance release of version 2.0.0. 
*   Aug 27, 2013: Release of variant tools 2.0. This is a major release of variant tools with many new features. Please check [ChangeLog][3] for details. 
...
[`vtools admin`][4] `--update_resource` and `vtools_report sequences`, and allows the use of arbitrary characters for names of variant tables. 
...
*   Sep 25, 2012: Release of variant tools version 1.0.3, with new features and improvements in [`vtools associate`][4], [`vtools update`][5], [`vtools phenotype`][6] and `vtools_report` commands. 
*   Jul 9th, 2012: Release of variant tools version 1.0.3rc1. Other than a few bug fixes and major performance improvements, this release introduces new commands [`vtools associate`][4] and [`vtools admin`][7], with more than 20 association tests implemented under a unified association test framework. 
*   Jan 24th, 2012: Release of variant tools version 1.0.2. This release fixes a major bug that causes duplicate output in commands [`vtools output`][8] and [`vtools export`][9] when range-based annotation databases are used. All users are recommended to upgrade. 
*   Jan 2nd, 2012: Release of variant tools version 1.0.1. This version contains a few new features and bug fixes, and more importantly, dramatic performance improvement for many commands. Please refer to [ChangeLog][3] for details about this release. 
*   Dec 30th, 2011: the gwasCatalog annotation source is available for download. See examples of how to use [gwasCatalog][10] to find published GWA hits that are near your variants. 
...
*   Dec 4th, 2011: An [application note][11] that describes variant tools has been published online in *Bioinformatics*. 
...
*   Oct 16, 2011: Release of release candidate version 1.0rc2. This version has a new option `--children` for command [`vtools init`][13], which allows the creation of a project by merging multiple subprojects. 
*   Oct 7, 2011: Release of release candidate version 1.0rc1. This version has a new [`vtools export`][9] command that can export in ANNOVAR and VCF formats. 
*   Sep 27, 2011: Release of the second beta. This version contains full Python 3 support and a much more powerful [`vtools import`][14] command. 
...
1.  `myvariants` contains a list of variants, which is a subset of the *master variant table* (all the variant of your project) and is typically created using command [`vtools select`][15]. 
2.  Command [`vtools output`][8] output information for all variants in `myvariant`, which include 
3.  `chr`, `pos`, `ref`, `alt` constitute a *variant*, namely location and type of a mutation. 
4.  `hom_case` and `hom_ctrl` are number of homozygous genotypes of this variant in cases and controls. These are called *variant info fields* and are added to the project using command [`vtools update`][5] `--from_stat` 
5.  `dbNSFP.SIFT_score`, `dbSNP.name` and `refGene.name2` are annotation information from different annotation databases. Annotation databases are not part of the project. They are connected to the project using command [`vtools use`][16]. 
6.  `asso1.p_value` and `asso2.p_value` are results of two different association analysis. These are annotation databases created by command [`vtools associate`][4]. 
7.  [`ref_sequence`][17] (chr, pos-5, pos+5)` is a function provided by *variant tools* to retrieve the reference sequence around the variant. Here 5 basepair of the up and downstream of each variant is returned. 
8.  [`track`][18] is a function to extract information from external files. In this example, the depth of coverage at the location of the variant in the specified bam file is returned. 
...
As you can see, individual commands such as [`vtools use`][16] and [`vtools update`][5] do not produce any output, but adds information to the project that can be displayed along with others. Then, it is important to remember that all such information can be used to select, prioritize, and analyze your variants. Another fabricated command would look like 
...
|**Catagory**|         **Tasks**       |
| ---------- | ----------------------- |
| Variant calling        | [Call variants from raw reads in FASTQ or BAM (convert to FASTQ first) formats using the GATK best practice pipeline.][20]                                                                                                                                         |
| Import variants        | [Import variants and genotypes in VCF format, with options to import specified variant and genotype info fields.][14]                                                                                                                                                 |
|                        | [Import all info and genotype fields, including customized fields from VCF files.][21]                                                                                                                                                                             |
...
|                        | [Pipeline to import variants from the recent versions of the Illumina CASAVA pipeline (in VCF format) that provides variant calls from two probabilistic models.][22]                                                                                              |
|                        | Import variants in [text][23] or [CSV][24] files.                                                                                                                                                                                                               |
|                        | Import variants from files in [Plink format][25]                                                                                                                                                                                                                     |
...
[1]: documentation/functions/genotype/
[2]: documentation/functions/samples/
[3]: development/changelog/
[4]: documentation/vtools-commands/associate/
[5]: documentation/vtools-commands/update/
[6]: documentation/vtools-commands/phenotype/
[7]: documentation/vtools-commands/admin/
[8]: documentation/vtools-commands/output/
[9]: documentation/vtools-commands/export/
[10]: applications/annotation/variants/gwas/
[11]: https://academic.oup.com/bioinformatics/article/28/3/421/189037
[12]: applications/annotation/variants/exomesequencingpro/
[13]: documentation/vtools-commands/init/
[14]: documentation/vtools-commands/import/
[15]: documentation/vtools-commands/select/
[16]: documentation/vtools-commands/use/
[17]: documentation/functions/ref_sequence/
[18]: documentation/functions/track/
[19]: documentation/keyconcepts/
